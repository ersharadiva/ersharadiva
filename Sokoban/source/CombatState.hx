...
class CombatState extends FlxSubState
{
	...
	private var _btnEscape:FlxButton;
	...
	public function new() 
	{
		...
		_sprPlayer.animation.frameIndex = 11;
		...
		_btnAttack = new FlxButton(_sprBack.x + 30, _sprBack.y + 48, "Attack!", playerAttack);
		_btnAttack.scrollFactor.set();
		add(_btnAttack);
		_btnEscape = new FlxButton(_sprBack.x + 30, _btnAttack.y + 20, "Escape", playerEscape);
		_btnEscape.scrollFactor.set();
		add(_btnEscape);
		...
		_sprEnemy.scrollFactor.set();
		_sprPlayer.scrollFactor.set();
		_sprScreen.scrollFactor.set();
		_sprWave.scrollFactor.set();
		_txtPlayerHealth.scrollFactor.set();
		_results.scrollFactor.set();
		_enemyHealthBar.scrollFactor.set();
	...
	private function playerEscape():Void
	{
		_selected = 1;
		makeChoice();
	}
	...
	{
		_alpha = Value;
		_sprBack.alpha = _alpha;
		_sprEnemy.alpha = _alpha;
		_sprPlayer.alpha = _alpha;
		_sprScreen.alpha = _alpha;
		_sprWave.alpha = _alpha;
		_btnAttack.alpha = _alpha;
		_btnEscape.alpha = _alpha;
		_txtPlayerHealth.alpha = _alpha;
		_results.alpha = _alpha;
		_enemyHealthBar.alpha = _alpha;
